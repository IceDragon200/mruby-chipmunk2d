...
MRB_CP_EXTERN mrb_cp_body_user_data*
mrb_cp_body_user_data_new(mrb_state* mrb)
{
  mrb_cp_body_user_data* user_data =
    (mrb_cp_body_user_data*)mrb_malloc(mrb, sizeof(mrb_cp_body_user_data));
...
MRB_CP_EXTERN const struct mrb_data_type mrb_cp_body_type = { "cpBody", mrb_cp_body_free };
...
body_initialize_userdata(mrb_state* mrb, mrb_value self)
{
  cpBody* body;
  mrb_cp_body_user_data* user_data;
  body = mrb_cp_get_body_ptr(mrb, self);
...
body_each_shape_func(cpBody* body, cpShape* shape, void* data)
{
  ...
  body_user_data = (struct mrb_cp_body_user_data*)cpBodyGetUserData(body);
  shape_user_data = (struct mrb_cp_shape_user_data*)cpShapeGetUserData(shape);
